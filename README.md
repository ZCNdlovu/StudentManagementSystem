...
•	Closed for Modification: As I decided to add a "Part Time Student," I did not rewrite or edit the code inside my Student class, instead i
Open for Extension: I simply create a new class "Part Time Student," that extends Student and implements its own calculateTuition() logic.
...
•	Application: In a Main class, i created a List<Student>. Whether i added an UndergraduateStudent, GraduateStudent and PartTimeStudent to that list, calling .calculateTuition() will always work and return a valid double. The program doesn't "break" because the subclass fulfills the promise made by the parent class.
